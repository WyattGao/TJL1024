...
- (void)commonInit
{
    ...
    _overlayView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 73)];
    [_overlayView setUserInteractionEnabled:NO];
    
    // Set the default frame size
    // Don't worry, we will be changing this later
    _dialView = [[TRSDialView alloc] initWithFrame:CGRectMake(0,0, self.bounds.size.width, 73)];
    NSLog(@"%@",NSStringFromCGRect(_dialView.frame));
    ...
    _scrollView.contentSize = CGSizeMake(_dialView.frame.size.width, 0);
...
- (void)setDialRangeFrom:(NSInteger)from to:(NSInteger)to {
    ...
    self.scrollView.contentSize = CGSizeMake(self.dialView.frame.size.width,0);
}
...
-(void)setBorderColor:(UIColor *)layColor{
    self.layer.borderColor = layColor.CGColor;
}

-(void)setZoom:(NSInteger)zoom{
    self.dialView.zoom = zoom;
}
